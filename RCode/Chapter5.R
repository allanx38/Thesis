...
plot.ts(Mkt_train,
        ...
        xlim=c(2, 3200))
lines(meanf(Mkt_train, h=350) $mean, lty=2)
lines(rwf(Mkt_train,drift=TRUE,h=350)$mean,lty=3)
legend("bottomright",lty=c(2,3),
...
dat <- res_mean[,std6]
dig <- 2
cap =  c('Results from trading the predictions generated by a mean exponential smoothing system.',
         'Results from trading the predictions generated by a mean exponential smoothing system')
lab = 'tab:es_mean_sys'
...
dat <- res_drift[,std6]
dig <- 2
cap =  c('Results from trading the predictions generated by a drift exponential smoothing system.',
         'Results from trading the predictions generated by a drift exponential smoothing system')
...
# loop thru data sets - takes while so need to run just once
#run_exp_sm(fil,nm)
...
dat <- res_es[,std6]
dig <- 2
cap =  c('Results from trading the predictions generated by a moving window exponential smoothing system.',
         'Results from trading the predictions generated by a moving window exponential smoothing system')
lab = 'tab:es_sys'
...
# run the fnc ts_1 and apply Sys 1 to the auto.arima data
...
dat <- diff_df1
dig <- 0
cap <- c("PL from Naive Reversing system subtracted from results generated by a trading system based on ARIMA forecasts.",
         "ARIMA system results minus Naive Reversing results")
...
res3[misc_col] <- 'ARIMA/ANN Closing Price System 1'
...
dat <- res_diff3
dig <- 0
cap <- c("PL from Naive Reversing system subtracted from results generated by trading System 1 based on ARIMA/ANN closing price forecasts.",
         "ARIMA/ANN closing price system results minus Naive Reversing results")
...
res5[misc_col] <- 'ARIMA/k-NN Closing Price System 1'
...
# produce latex table from ts_1
dat <- res_diff5[,c(1,5,7,8,10)]
dig <- 0
cap <- c("PL from Naive Reversing system subtracted from results generated by trading System 1 based on ARIMA/k-NN closing price forecasts.",
         "ARIMA/k-NN closing price system results minus Naive Reversing results")
...
fil <- c("../Data/ARIMA2/PredUpDn_CAT/ar_334_UD_ann_Dax2.csv",
...
# produce latex table from ts_1
dat <- res_diff8[,c(1,5,7,8,10)]
dig <- 0
cap <- c("PL from Naive Reversing system subtracted from results generated by a trading system based on ARIMA/k-NN U/D forecasts.",
         "ARIMA/k-NN U/D system results minus Naive Reversing results")
...
# comp aring to Naive Prev
res_diff9 <- sub_df(res9,NaiveRev)

# produce latex table from ts_1
dat <- res_diff9[,c(1,5,7,8,10)]
dig <- 0
cap <- c("PL from Naive Reversing system subtracted from results generated by a trading system based on ARIMA/SVM U/D forecasts.",
         "ARIMA/SVM U/D system results minus Naive Reversing results")
lab = 'tab:chp_ts:pUD_CAT_arima_svm_sys_diff'
filname ='../Tables/chp_ts_predUpDn_CAT_arima_svm_sys_diff.tex'
inclrnam=FALSE
print_xt(dat,dig,cap,lab,al,filname,inclrnam)
