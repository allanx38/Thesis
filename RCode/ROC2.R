roc_sys2 <- function(Mkt, SLoss, MktName){
  ...
  # Trade Long
  Mkt$Long <- ifelse(Mkt$prevROC > 0,Mkt$Close-Mkt$Open,NA)
  results["LongPL"] <- round(sum(Mkt$Long, na.rm=TRUE))
  #Adj for SLoss
  if (SLoss < 0) {
    Mkt$Long <- ifelse(Mkt$preC1 > Mkt$preC2,
  ...
  # Trade Short
  Mkt$Short <- ifelse(Mkt$prevROC < 0,Mkt$Open-Mkt$Close,NA)
  results["ShortPL"] <- round(sum(Mkt$Short, na.rm=TRUE))
  #Adj for SLoss
  if (SLoss < 0) {
    Mkt$Short <- ifelse(Mkt$preC1 < Mkt$preC2,
